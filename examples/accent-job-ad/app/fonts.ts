...
/**
 * TrimPoster Fat — loaded from /public.
 */
export const trimPosterFat: Font = async () => ({
  name: "TrimPoster",
  data: await fetchFont(
    `${process.env.BASE_URL ?? "http://localhost:3839"}/TrimPoster-Fat.ttf`,
  ),
  weight: 700,
  style: "normal",
});
...
/**
 * Attila Sans Sharp Bold — loaded from /public.
 */
export const attilaSansSharpBold: Font = async () => ({
  name: "AttilaSansSharp",
  data: await fetchFont(
    `${process.env.BASE_URL ?? "http://localhost:3839"}/AttilaSansSharp-Bold.otf`,
  ),
  weight: 700,
  style: "normal",
});
