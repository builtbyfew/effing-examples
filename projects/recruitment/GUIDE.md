...
`app/fonts.ts` defines the project's font helpers — a small `loadFonts(fonts)` utility and the brand's Google Fonts ready to import:
...
const fonts = await loadFonts([spaceGroteskBold, caveatBold]);
```
